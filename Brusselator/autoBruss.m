...
% Abril 2016
%
% Ventana  para la simulación de la autocorrelación del sistema Brusselator

function varargout = autoBruss(varargin)
    % AUTOBRUSS MATLAB code for autoBruss.fig
    %      AUTOBRUSS, by itself, creates a new AUTOBRUSS or raises the existing
    %      singleton*.
    %
    %      H = AUTOBRUSS returns the handle to a new AUTOBRUSS or the handle to
    %      the existing singleton*.
    %
    %      AUTOBRUSS('CALLBACK',hObject,eventData,handles,...) calls the local
    %      function named CALLBACK in AUTOBRUSS.M with the given input arguments.
    %
    %      AUTOBRUSS('Property','Value',...) creates a new AUTOBRUSS or raises the
    %      existing singleton*.  Starting from the left, property value pairs are
    %      applied to the GUI before autoBruss_OpeningFcn gets called.  An
    %      unrecognized property name or invalid value makes property application
    %      stop.  All inputs are passed to autoBruss_OpeningFcn via varargin.
    %
    ...
    % Edit the above text to modify the response to help autoBruss

    % Last Modified by GUIDE v2.5 16-Apr-2016 13:03:41

    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
                       'gui_Singleton',  gui_Singleton, ...
                       'gui_OpeningFcn', @autoBruss_OpeningFcn, ...
                       'gui_OutputFcn',  @autoBruss_OutputFcn, ...
                       'gui_LayoutFcn',  [] , ...
                       'gui_Callback',   []);
...
% --- Executes just before autoBruss is made visible.
function autoBruss_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to autoBruss (see VARARGIN)

    % Choose default command line output for autoBruss
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

    % UIWAIT makes autoBruss wait for user response (see UIRESUME)
    % uiwait(handles.figure1);
    set(handles.figure1,'CloseRequestFcn',@cerrar);
end

% --- Outputs from this function are returned to the command line.
function varargout = autoBruss_OutputFcn(hObject, eventdata, handles) 
...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    pintaAutocorrelacion (a, b, m, w, v1_0, v2_0, v3_0, comp);
