...
% Abril 2016
%
% Ventana  para la simulación del mapa de Poincaré del sistema Brusselator

function varargout = poincareBruss(varargin)
    % POINCAREBRUSS MATLAB code for poincareBruss.fig
    %      POINCAREBRUSS, by itself, creates a new POINCAREBRUSS or raises the existing
    %      singleton*.
    %
    %      H = POINCAREBRUSS returns the handle to a new POINCAREBRUSS or the handle to
    %      the existing singleton*.
    %
    %      POINCAREBRUSS('CALLBACK',hObject,eventData,handles,...) calls the local
    %      function named CALLBACK in POINCAREBRUSS.M with the given input arguments.
    %
    %      POINCAREBRUSS('Property','Value',...) creates a new POINCAREBRUSS or raises the
    %      existing singleton*.  Starting from the left, property value pairs are
    %      applied to the GUI before poincareBruss_OpeningFcn gets called.  An
    %      unrecognized property name or invalid value makes property application
    %      stop.  All inputs are passed to poincareBruss_OpeningFcn via varargin.
    %
    ...
    % Edit the above text to modify the response to help poincareBruss

    % Last Modified by GUIDE v2.5 16-Apr-2016 15:18:15

    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
                       'gui_Singleton',  gui_Singleton, ...
                       'gui_OpeningFcn', @poincareBruss_OpeningFcn, ...
                       'gui_OutputFcn',  @poincareBruss_OutputFcn, ...
                       'gui_LayoutFcn',  [] , ...
                       'gui_Callback',   []);
...
% --- Executes just before poincareBruss is made visible.
function poincareBruss_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to poincareBruss (see VARARGIN)

    % Choose default command line output for poincareBruss
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

    % UIWAIT makes poincareBruss wait for user response (see UIRESUME)
    % uiwait(handles.figure1);
    set(handles.figure1,'CloseRequestFcn',@cerrar);
end

% --- Outputs from this function are returned to the command line.
function varargout = poincareBruss_OutputFcn(hObject, eventdata, handles) 
...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    cortaPoincareBrusselator ([n1, n2, n3], [pt1, pt2, pt3], a, b, m, w, v1_0, v2_0, v3_0);
