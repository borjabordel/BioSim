...
% Abril 2016
%
% Ventana para configurar la simulación temporal del corazón

function varargout = temporalCorazon(varargin)
    % TEMPORALCORAZON MATLAB code for temporalCorazon.fig
    %      TEMPORALCORAZON, by itself, creates a new TEMPORALCORAZON or raises the existing
    %      singleton*.
    %
    %      H = TEMPORALCORAZON returns the handle to a new TEMPORALCORAZON or the handle to
    %      the existing singleton*.
    %
    %      TEMPORALCORAZON('CALLBACK',hObject,eventData,handles,...) calls the local
    %      function named CALLBACK in TEMPORALCORAZON.M with the given input arguments.
    %
    %      TEMPORALCORAZON('Property','Value',...) creates a new TEMPORALCORAZON or raises the
    %      existing singleton*.  Starting from the left, property value pairs are
    %      applied to the GUI before temporalCorazon_OpeningFcn gets called.  An
    %      unrecognized property name or invalid value makes property application
    %      stop.  All inputs are passed to temporalCorazon_OpeningFcn via varargin.
    %
    ...
    % Edit the above text to modify the response to help temporalCorazon

    % Last Modified by GUIDE v2.5 01-Apr-2016 22:06:42

    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
                       'gui_Singleton',  gui_Singleton, ...
                       'gui_OpeningFcn', @temporalCorazon_OpeningFcn, ...
                       'gui_OutputFcn',  @temporalCorazon_OutputFcn, ...
                       'gui_LayoutFcn',  [] , ...
                       'gui_Callback',   []);
...
% --- Executes just before temporalCorazon is made visible.
function temporalCorazon_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to temporalCorazon (see VARARGIN)

    % Choose default command line output for temporalCorazon
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

    % UIWAIT makes temporalCorazon wait for user response (see UIRESUME)
    % uiwait(handles.figure1);
    set(handles.figure1,'CloseRequestFcn',@cerrar);
end

% --- Outputs from this function are returned to the command line.
function varargout = temporalCorazon_OutputFcn(hObject, eventdata, handles) 
...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    if(isnan(T*v1_0*v2_0*v3_0*v4_0))
    ...
    cd ('Temporal');
    PintaEvolucionTemporal(k, w1, w2, a, f, b1, b2, c1, c2, v1_0, v2_0, v3_0, v4_0, T);
    cd('..');
    close temporalCorazon;
    abrirCerrojo();
