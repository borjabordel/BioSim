...
% Abril 2016
%
% Ventana para obtener la pulsación de torsión del corazón

function varargout = torsionCorazon(varargin)
    % TORSIONCORAZON MATLAB code for torsionCorazon.fig
    %      TORSIONCORAZON, by itself, creates a new TORSIONCORAZON or raises the existing
    %      singleton*.
    %
    %      H = TORSIONCORAZON returns the handle to a new TORSIONCORAZON or the handle to
    %      the existing singleton*.
    %
    %      TORSIONCORAZON('CALLBACK',hObject,eventData,handles,...) calls the local
    %      function named CALLBACK in TORSIONCORAZON.M with the given input arguments.
    %
    %      TORSIONCORAZON('Property','Value',...) creates a new TORSIONCORAZON or raises the
    %      existing singleton*.  Starting from the left, property value pairs are
    %      applied to the GUI before torsionCorazon_OpeningFcn gets called.  An
    %      unrecognized property name or invalid value makes property application
    %      stop.  All inputs are passed to torsionCorazon_OpeningFcn via varargin.
    %
    ...
    % Edit the above text to modify the response to help torsionCorazon

    % Last Modified by GUIDE v2.5 13-Apr-2016 13:04:59

    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
                       'gui_Singleton',  gui_Singleton, ...
                       'gui_OpeningFcn', @torsionCorazon_OpeningFcn, ...
                       'gui_OutputFcn',  @torsionCorazon_OutputFcn, ...
                       'gui_LayoutFcn',  [] , ...
                       'gui_Callback',   []);
...
% --- Executes just before torsionCorazon is made visible.
function torsionCorazon_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to torsionCorazon (see VARARGIN)

    % Choose default command line output for torsionCorazon
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

    % UIWAIT makes torsionCorazon wait for user response (see UIRESUME)
    % uiwait(handles.figure1);
    set(handles.figure1,'CloseRequestFcn',@cerrar);
end

% --- Outputs from this function are returned to the command line.
function varargout = torsionCorazon_OutputFcn(hObject, eventdata, handles) 
...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    cd('Torsion');
     ...
     if (s1)
       EvolucionPulsacionTorsion (NaN, c2, k, w1, w2, a, f, b1, b2, [v1_0,v2_0, v3_0, v4_0], min, max, num);
    else if (s2)
            EvolucionPulsacionTorsion (c1, NaN, k, w1, w2, a, f, b1, b2, [v1_0,v2_0, v3_0, v4_0], min, max, num);
        end
     end
     
     cd('..');

    close torsionCorazon;
    abrirCerrojo();
end
