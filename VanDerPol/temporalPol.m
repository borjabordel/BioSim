...
% Febrero 2016
%
% Ventana para la lanzar la simulación temporal de Van Der Pol (forzado o no)

function varargout = temporalPol(varargin)
    % TEMPORALPOL MATLAB code for temporalPol.fig
    %      TEMPORALPOL, by itself, creates a new TEMPORALPOL or raises the existing
    %      singleton*.
    %
    %      H = TEMPORALPOL returns the handle to a new TEMPORALPOL or the handle to
    %      the existing singleton*.
    %
    %      TEMPORALPOL('CALLBACK',hObject,eventData,handles,...) calls the local
    %      function named CALLBACK in TEMPORALPOL.M with the given input arguments.
    %
    %      TEMPORALPOL('Property','Value',...) creates a new TEMPORALPOL or raises the
    %      existing singleton*.  Starting from the left, property value pairs are
    %      applied to the GUI before temporalPol_OpeningFcn gets called.  An
    %      unrecognized property name or invalid value makes property application
    %      stop.  All inputs are passed to temporalPol_OpeningFcn via varargin.
    %
    ...
    % Edit the above text to modify the response to help temporalPol

    % Last Modified by GUIDE v2.5 29-Feb-2016 00:13:48

    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
                       'gui_Singleton',  gui_Singleton, ...
                       'gui_OpeningFcn', @temporalPol_OpeningFcn, ...
                       'gui_OutputFcn',  @temporalPol_OutputFcn, ...
                       'gui_LayoutFcn',  [] , ...
                       'gui_Callback',   []);
...
% --- Executes just before temporalPol is made visible.
function temporalPol_OpeningFcn(hObject, eventdata, handles, varargin)
    % This function has no output args, see OutputFcn.
    % hObject    handle to figure
    % eventdata  reserved - to be defined in a future version of MATLAB
    % handles    structure with handles and user data (see GUIDATA)
    % varargin   command line arguments to temporalPol (see VARARGIN)

    % Choose default command line output for temporalPol
    handles.output = hObject;

    % Update handles structure
    guidata(hObject, handles);

    % UIWAIT makes temporalPol wait for user response (see UIRESUME)
    % uiwait(handles.figure1);
    set(handles.figure1,'CloseRequestFcn',@cerrar);
end

% --- Outputs from this function are returned to the command line.
function varargout = temporalPol_OutputFcn(hObject, eventdata, handles) 
...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    PintaVanDerPolForzado(u, A, w, v1_0, v2_0, v3_0, T, caso);
